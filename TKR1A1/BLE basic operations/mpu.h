...
void updateSpeed(int timePassed, BLECharacteristic characteristic, int MPU) {
  ...
  float SpZ = (AcZ / 16384.0) * 9.81 * (timePassed * 0.001);  // AcZ*dT
...
void updateGyroscope(BLECharacteristic characteristic, int MPU) {
  ...
  int eulers[2];
  eulers[0] = atan2(AcX, sqrt(AcY * AcY + AcZ * AcZ)) * 57.3;
  eulers[1] = atan2(AcY, AcZ) * 57.3;

  // Send 2x eulers over bluetooth as 1x byte array
  characteristic.setValue((byte *)&eulers, 12);

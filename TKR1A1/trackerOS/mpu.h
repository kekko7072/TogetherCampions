...
void updateMPUAcceleration(int timestamp, BLECharacteristic accelerometer) {
  Wire.beginTransmission(MPU_ADDR);
  ...
  //Save on SDCARD
  sdcard_save("MPU_ACCELERATION;" + String(acc[0]) + ";" + String(acc[1]) + ";" + String(acc[2]) + ";" + String(acc[3]));
}
...
void updateMPUGyroscope(int timestamp, BLECharacteristic gyroscope) {
  ...
  //Save on SDCARD
  sdcard_save("MPU_GYROSCOPE;" + String(gyr[0]) + ";" + String(gyr[1]) + ";" + String(gyr[2]) + ";" + String(gyr[3]));
}
